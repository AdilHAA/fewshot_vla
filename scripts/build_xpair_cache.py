...
def build_records(episodes, encoder, mean, std, sent_encode, num_frames, stride,
                  n_color, beta_t, beta_f, encoder_id, sent_id):
    """Pure orchestration over an iterable of episode dicts. Returns
    (clips ndarray fp16, keys Tensor, records list[dict], CacheHeader)."""
    clips_list, keys_list, records = [], [], []
    ntok = d_enc = d_text = None
    row = 0
    for ep in episodes:
        ...
        for variant in range(1 + n_color):
            ...
            toks = dino_encode(encoder, mean, std, clip.unsqueeze(0))[0]  # (T*Ntok,D)
            if ntok is None:
                d_enc = toks.shape[-1]
                ntok = toks.shape[0] // num_frames
    ...
    header = CacheHeader(
        ...
        jitter="chan-gain-bias", d_enc=d_enc, ntok=ntok, sentence_encoder_id=sent_id,
        beta_t=beta_t, beta_f=beta_f, d_text=d_text, d_frame=d_enc, n_clips=row)
    return clips, keys, records, header


def _load_episodes(repo_id, revision, num_frames, stride):  # pragma: no cover
    """GPU/dataset adapter — implement against your installed LeRobotDataset version.
    Must yield dicts: {frames: Tensor(n,C,H,W) in [0,1], instruction: str,
    episode: int, task_index: int, object_set: str}."""
    from src.data.libero import prefetch_all_data_parquets
    prefetch_all_data_parquets(repo_id, revision)
    raise NotImplementedError(
        "Implement _load_episodes against your installed LeRobotDataset version; "
        "it must yield the episode dicts documented in this function's docstring.")
...
def main(argv=None):  # pragma: no cover
    ...
    p.add_argument("--encoder", default="dino")
    ...
    mean, std = imagenet_buffers(device, next(dino.parameters()).dtype)
    ...
    clips, keys, records, header = build_records(
        episodes, dino, mean, std, sent_encode, args.num_frames, args.stride,
